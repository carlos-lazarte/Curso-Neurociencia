# 🧠 Curso de Neurociencia del Desarrollo

Curso progresivo para desarrollar una comprensión experta de la neurociencia del desarrollo, desde los fundamentos celulares hasta los circuitos, la plasticidad, la cognición y la conducta.

## Progreso actual

### Nivel I — Fundamentos

#### Módulo 1 — Introducción a la Neurociencia

- [x] Clase 1.1 — ¿Qué es la neurociencia?
- [x] Clase 1.2 — Historia y evolución de la neurociencia
- [x] Clase 1.3 — Ramas de la neurociencia y niveles de análisis
- [x] Clase 1.4 — La neurona
- [x] Clase 1.5 — Las células gliales
- [x] Clase 1.6 — Comunicación neuronal: sinapsis, neurotransmisores y receptores
- [x] Clase 1.7 — Neurotransmisores y neuromoduladores: la química del cerebro
- [ ] Clase 1.8 — Neuroplasticidad: cómo cambia el cerebro con la experiencia

## Estructura

```text
NEUROCIENCIA DEL DESARROLLO
│
├── NIVEL I — FUNDAMENTOS
│   └── MÓDULO 1 — INTRODUCCIÓN A LA NEUROCIENCIA
│       ├── 1.1 Neurociencia
│       ├── 1.2 Historia
│       ├── 1.3 Ramas y niveles de análisis
│       ├── 1.4 Neurona
│       ├── 1.5 Glía
│       ├── 1.6 Comunicación neuronal
│       ├── 1.7 Neurotransmisores y neuromoduladores
│       └── 1.8 Neuroplasticidad
```

> Curso en desarrollo. Las clases se irán incorporando progresivamente al repositorio.
